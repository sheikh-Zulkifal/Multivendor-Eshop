...
const DashboardHero = () => {
  ...
  const totalEarningsWithoutTax = deliveredOrders && deliveredOrders.reduce((acc, item) => acc + item.totalPrice, 0) ;

  const serviceCharge = totalEarningsWithoutTax * 0.1

  // Guard against seller or availableBalance being undefined
  ...
  const columns = [
...

    {
      ...
      cellClassName: (params) => {
        // `params.getValue` is not available here in this DataGrid version.
        // Use `params.value` (the cell value) which holds the status for this column.
